...
p=predColl.add_pred(dname='sort' ,arguments=['L','L'] , variables=['L2','L2']    , pFunc = DNF('sort',terms=2,init=[-1,.1,-1,.1],sig=2) ,arg_funcs=['tH'])
...
for item in Ls:
    bg.add_example('sort',  (item, ''.join(sorted(item))  ) )

# for item in L:
    if len(item)<2:
        bg.add_backgroud('sort', (item,''.join(sorted(item)) ) )
