...
# parser.add_argument('--RATIO_POS',default=0.5,help='Batch Size',type=float)
...
parser.add_argument('--LR_SC', default={ (-1000,2):.005 ,  (2,1e5):.005} , help='Learning rate schedule',type=dict)
...
parser.add_argument('--PLOGENT', default=.35 , help='Crossentropy coefficient',type=float)
...
parser.add_argument('--ITER', default=1250, help='Maximum number of iteration',type=int)
...
predColl.add_pred(dname='tempAdvisedBy',arguments=['P','P'] )
...
predColl.add_pred(dname='advisedBy1',oname='advisedBy', arguments=['P','P'] , variables=['C'   ] ,pFunc = DNF('advisedBy1',terms=2,init=[-1,.1,-1,.1],sig=2)  , use_neg=True , Fam='or',chunk_count=0)
predColl.add_pred(dname='advisedBy2',oname='advisedBy', arguments=['P','P'] , variables=[   ] ,pFunc = DNF('advisedBy2',terms=2,init=[-1,.1,-1,.1],sig=2)  , use_neg=True , Fam='max',chunk_count=0)
